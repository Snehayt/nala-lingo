...
Actions:
  add (default)     Add new variants (skip existing)
  update            Update existing variants (and add new)
  refresh           Rebuild all variants from page (wipe + regenerate)
...
Examples:
  ...
  npm run nala-test-gen https://page.html refresh
  npm run nala-test-gen https://page.html delete ax-columns width-2-columns
`);
  process.exit(1);
}
...
// === SMART PARSER ===
function parseBlock(block, blockName, variantName, idx, input) {
  const blockPrefix = blockName;
  const nestedEls = [...block.querySelectorAll(`[class^="${blockPrefix}-"]`)];
  const typeCounters = {};
  ...
  const structuredData = nestedEls
    .map((el) => {
      ...
      // Maintain nth counters per type
  ...
  return {
    ...
    selector: `.${[blockName, variantName].join('.')}`,
...
(async () => {
  ...
  const touchedBlocks = new Set(); // <— collect blocks we touched to build files after loop
  ...
  for (const [idx, block] of blocks.entries()) {
    ...
    let schema = fs.existsSync(blockJsonFile)
    ...
    const newVariant = parseBlock(block, blockName, variantName, variantIndex, input);

    // Preserve tcid when updating
    if (existingIndex >= 0 && schema.variants[existingIndex].tcid) {
      newVariant.tcid = schema.variants[existingIndex].tcid;
    }

    // === Command logic ===
    if (action === 'refresh') {
      schema = { block: blockName, variants: [newVariant] };
      updatedCount++;
      ...
      if (existingIndex >= 0) {
        schema.variants.splice(existingIndex, 1);
        deletedCount++;
        console.log(chalk.red(`🗑️ Deleted variant: ${variantName}`));
      }
    } else if (action === 'add') {
      if (existingIndex >= 0) {
        skippedCount++;
    ...
    touchedBlocks.add(blockName); // mark for file creation after loop
  }

  // === After processing all blocks/variants, (now) create spec/page/test ===
...
`;
  ...
  constructor(page, nth = 0) {
    this.page = page;
    this.block = page.locator('.${blockName}').nth(nth);
...
`;
    ...
    if (!fs.existsSync(blockTestFile)) {
  ...
  test(\`[Test Id - \${features[${i}].tcid}] \${features[${i}].name},\${features[${i}].tags}\`, async ({ page, baseURL }) => {
    ...
    const block = new ${safeClassName}Block(page);
    ...
    await test.step('Step 2: Verify block content', async () => {
      ...
      if (data.headings?.length) for (let j = 0; j < data.headings.length; j++)
        await expect(block.headings.nth(j)).toContainText(data.headings[j]);
      if (data.paragraphs?.length) for (let j = 0; j < data.paragraphs.length; j++)
    ...
    await test.step('Step 3: Verify analytics', async () => {
      if (data.analytics)
        await expect(block.block).toHaveAttribute('daa-lh', data.analytics);
    });
...
test.describe('Express ${safeClassName} Block test suite', () => {
${variantsCode}
});
`
